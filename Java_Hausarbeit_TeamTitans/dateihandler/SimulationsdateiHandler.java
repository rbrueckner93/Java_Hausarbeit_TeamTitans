...
/**
 * Erstellt aus einem uebergebenen Simulator eine Textdatei, die alle Flugrouten
 * mit ihrer spez. Reiseliste enthaelt.
 * 
 * @author TolleN
 */
public class SimulationsdateiHandler extends Datei {
	 ...
	 * 
	 * @return ArrayList<String> ,jedes Element entspricht einer Zeile.
	 */
	private ArrayList<String> erstelleOutputStreamSim() {
		...
		for (Flugroute aktuelleFlugroute : fertigeSimulation.getRouten()) {
			...
			fertigerText.add(MERKMAL_BEGINN + BEZEICHNER_START
					...
					+ aktuelleFlugroute.getHerkunft().getName() + MERKMAL_ENDE);
			fertigerText.add(MERKMAL_BEGINN + BEZEICHNER_ZIEL
					+ BEZEICHNER_WERT_TRENNER
					+ aktuelleFlugroute.getZiel().getName() + MERKMAL_ENDE);
			fertigerText.add(MERKMAL_BEGINN + BEZEICHNER_REISELISTE
					+ BEZEICHNER_WERT_TRENNER
